...
public class ChatController {
    ...
    public ApiResponse<ChatMessageDto> chatLLM(
            @AuthenticationPrincipal PrincipalOAuth2User principal,
            @RequestBody ChatRequestDto chatRequestDto
    ) {
        return ApiResponse.onSuccess(chatService.ragChat(principal.getId(), chatRequestDto));
    }

    @PostMapping(value = "/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    @Operation(summary = "LLM 채팅 스트리밍", description = "RAG 기반으로 SSE 스트리밍 응답을 제공합니다.")
    public SseEmitter chatLLMStream(
            @AuthenticationPrincipal PrincipalOAuth2User principal,
            @RequestBody ChatRequestDto chatRequestDto
    ) {
        return chatService.streamRagChat(principal.getId(), chatRequestDto);
    }
